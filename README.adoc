...
docker images
docker run  --cap-add=NET_ADMIN --name <C-NAME> -itd -p <HOST-PORT>:<C-PORT> <REPO-NAME>:<TAG-NAME>
docker ps -a
# 不要なコンテナ、イメージ削除
docker rm -f <CID>
docker rmi -f <IID>
...
* 「--cap-add=NET_ADMIN」の代わりに「--privileged」でもopenvpnのエラー(errno=1)を回避できるが、 +
...
----
gobuster dir \                    # ディレクトリ/ファイルの総当りモード
         -u http://[TargetIP]/ \
         -e \                     # URL全体を表示
         -k \                     # SSL証明書の検証をスキップ
...
----
enum4linux -a [TargetIP] | tee ./enum4linux.log
----
